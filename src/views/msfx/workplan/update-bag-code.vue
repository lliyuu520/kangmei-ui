<template>
	<el-dialog v-model="visible" title="修改盒码'" :close-on-click-modal="false" draggable>
...
import { updateBagCodeApi } from '@/api/msfx/workplan'
...
const init = (data: any) => {
	...
	// 通过data传递数据
	dataForm.bagCodeId = data.bagCodeId
	dataForm.batchNo = data.batchNo
	dataForm.bagCode = data.bagCode
}
...
const submitHandle = () => {
	dataFormRef.value.validate((valid: boolean) => {
		...
		updateBagCodeApi(dataForm).then(() => {
			ElMessage.success({
				message: '操作成功',
				duration: 500,
				onClose: () => {
